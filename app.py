...
if opcao == "📊 Dashboard Principal":
    ...
    if not df_dados.empty:
        ...
        if st.button("Guardar Alterações na Planilha"):
            try:
                ...
                # Se houver filtros aplicados, atualizar integrando com o DataFrame original pode ser complexo. 
                # Recomendamos remover a coluna temporária "Selecionar" do df alterado e atualizar a planilha completa.
                df_para_salvar = edited_df.drop(columns=["Selecionar"], errors="ignore")
                
                # Nota: Caso queira actualizar apenas as linhas editadas mantendo as filtradas, 
                # aqui enviamos os dados completos atuais do cache ou unificados.
                sheet.clear()
    ...
    if not df_custos.empty:
        ...
        st.info("Nenhum dado de custos encontrado na aba 'Controle_Custos'.")import logging
import re
import gspread
import pandas as pd
import plotly.express as px
