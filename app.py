...
st.set_page_config(
    page_title="Gestão de Pontos - Locais Noturnos",
    page_icon="🗺️",
    layout="wide"
)
...
opcao = st.sidebar.selectbox(
    "Escolha a Seção:",
    ["🗺️ Visualizar Mapa de Pontos", "📋 Tabela de Dados e Ações", "🔒 Área Privada (Financeiro)"]
)
...
if df.empty:
    ...
    # --- ABA 3: ÁREA PRIVADA (FINANCEIRO / EXCEÇÃO DE LOCAIS NOTURNOS) ---
    elif opcao == "🔒 Área Privada (Financeiro)":
        st.subheader("🔒 Painel Privado - Cálculo de Repasse (Locais Noturnos)")
        st.markdown("---")
        ...
        if senha_digitada == SENHA_MESTRE:
            ...
            if "Status" in df.columns:
                for _, row in df.iterrows():
                    st_val = str(row.get("Status", "")).strip().capitalize()
                    if st_val == "Auditado":
                        total_auditados += 1
                    elif st_val == "Justificado":
                        total_justificados += 1
            
            valor_auditados = total_auditados * 35.0
            valor_justificados = total_justificados * 25.0
            valor_total_geral = valor_auditados + valor_justificados
            
            st.markdown(f"""
                <div class="card-privado">
                    <h3 style="margin-top:0; color: white;">📊 Resumo Financeiro Exclusivo (Locais Noturnos)</h3>
                    <p style="font-size: 16px; margin-bottom: 5px;">Regra de exceção aplicada:</p>
                    <ul style="font-size: 15px; margin-bottom: 15px;">
                        <li><b>Pontos Auditados (Exceção):</b> R$ 35,00 cada</li>
                        <li><b>Pontos Justificados:</b> R$ 25,00 cada</li>
                    </ul>
                    <hr style="border-color: rgba(255,255,255,0.2);">
                    <p style="font-size: 16px;">🟢 <b>Total Auditados:</b> {total_auditados} ponto(s) × R$ 35,00 = <b>R$ {valor_auditados:.2f}</b></p>
                    <p style="font-size: 16px;">🔵 <b>Total Justificados:</b> {total_justificados} ponto(s) × R$ 25,00 = <b>R$ {valor_justificados:.2f}</b></p>
                    <h2 style="margin-bottom: 0; color: #fef08a;">💰 Valor Total a Receber: R$ {valor_total_geral:.2f}</h2>
                </div>
            """, unsafe_allow_html=True)
            
            st.markdown("### 📋 Detalhamento dos Pontos Contabilizados")
            if "Status" in df.columns:
                df_calc = df[df["Status"].isin(["Auditado", "Justificado"])].copy()
                if not df_calc.empty:
                    df_calc["Valor Unitário (R$)"] = df_calc["Status"].apply(lambda x: 35.0 if str(x).strip().capitalize() == "Auditado" else 25.0)
                    cols_fin = [c for c in df_calc.columns if not c.startswith("_") and c != "Identificador_Unico"]
                    st.dataframe(df_calc[cols_fin], use_container_width=True)
                else:
                    st.info("Nenhum ponto com status 'Auditado' ou 'Justificado' encontrado no momento.")
            
        elif senha_digitada != "":
            st.error("❌ Senha incorreta. Tente novamente.")
        else:
            st.info("ℹ️ Insira a senha no campo acima para visualizar os dados financeiros confidenciais.")